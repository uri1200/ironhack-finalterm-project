<template>
  <div
    class="add-task w-4/5 flex flex-col justify-center m-auto shadow-2xl bg-blue-900 align-center rounded-3xl"
  >
    <div class="my-4">
      ...
      <div class="flex flex-col">
        <div class="input-field mt-4 mb-2 w-[90%] mx-auto">
          <input
            class="bg-slate-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"
            type="text"
        ...
        <button
          @click="addTask"
          class="button font-dosis text-xl py-3 px-7 rounded-lg self-center text-center text-white bg-teal-400 duration-200 border-lg border-transparent hover:border-white hover:bg-teal-500"
        >
          Add Task
        </button>
      </div>
...
const addTask = async () => {
  if (name.value.length <= 3 || description.value.length === 0) {
    ...
    errorMessage.value =
      "Error: The task title length has to be more than 3 or description is empty";
    setTimeout(() => {
